...
context "Database" do
  ...
  context "check_project_name" do
    asserts("ok") do
      project = dc.query("select `name` from `projects` where `id` = (select `project_id` from `jobs` where `id` = 2);").first['name']
      project == 'hudson'

      project = dc.query("select `name` from `projects` where `id` = (select `project_id` from `jobs` where `id` = 1);").first
      project.nil?
    end
  end 
